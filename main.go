...
func init() {
	flag.IntVarP(&width, "width", "w", 79, "line width")
}

// process parses source with the given Parser and renders the result to w.
func process(p Parser, source []byte, w io.Writer, width int) error {
	r, err := p.Parse(source)
	if err != nil {
		return err
	}
	return r.Render(w, width)
}
...
func processFile(p Parser, filename string) error {
	...
	if err := process(p, source, tmp, width); err != nil {
		tmp.Close()
...
func run() error {
	...
	if len(args) == 0 {
		...
		return process(p, source, os.Stdout, width)
	}
